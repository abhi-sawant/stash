...
export function BookmarksProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ;(async () => {
      const [bookmarks, collections, settings] = await Promise.all([loadBookmarks(), loadCollections(), loadSettings()])
  ...
  const scheduleUpload = useCallback(() => {
    ...
    uploadTimerRef.current = setTimeout(async () => {
      try {
        ...
        // Ignore upload errors — background sync will retry on next app focus
      }
    }, 3000)
  }, [])
